...
`msync` tries its best to do everything in UTF-8, but this can cause issues, especially on Windows, where a string can be UTF-8, UTF-16 (especially paths), or whatever code page your system has. Work is ongoing to fix this, especially since there won't be a good way to interpret UTF-8 strings as paths on Windows until C++20, but things seem to work alright for now.