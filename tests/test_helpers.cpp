...
void make_status_json(std::string_view id, std::string& to_append)
{
	...
	to_append += account;
	to_append += R"(, "media_attachments" : [] , "mentions" : [] , "tags" : [] , "emojis" : [] , "card" : null, "poll" : null})";
}
...
void make_notification_json(std::string_view id, std::string& to_append)
{
	...
	to_append += id;
	to_append += R"(", "type" : ")";
	to_append += notification_types[zero_to_n(notification_types.size() - 1)];
	to_append += R"(", "created_at" : "2020-07-19T06:26:55.101Z", "account" : )";
	to_append += account;
	to_append += R"(, "status" : )";
...
void make_dm_json(std::string_view id, std::string& to_append)
{
	to_append += R"({"id": ")";
	to_append += id;
	to_append += R"(", "unread" : true, "accounts" : [)";
	to_append += account;
	to_append += ',';
	to_append += account;
	to_append += R"(], "last_status" : )";
