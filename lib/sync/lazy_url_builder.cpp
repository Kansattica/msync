...
std::string& lazy_url_builder::make_if_empty(std::string& field, std::string_view route)
{
	if (field.empty())
...
const std::string& lazy_url_builder::status_url()
{
	return make_if_empty(cached_status_url, STATUS_ROUTE);
}

const std::string& lazy_url_builder::media_url()
{
