...
parse_result parse(const int argc, const char *argv[], const bool silent)
{
    ...
    auto queueMode = (command("queue").set(ret.selected, mode::queue).doc("Manage the queue of things to send.") &
                      one_of(
                          option("-r", "--remove").set(ret.removeFromQueue, true).doc("Remove the post ids or filenames from the queue instead of adding them."),
                          option("-c", "--clear").set(ret.clearQueue, true).doc("Remove everything in the specified queue.")).doc("queue options") &
                      one_of(
                          ...
                          command("post").set(ret.queueaction, to_queue::post).doc("queue files to be posted (see generate)") & values("filenames", ret.queued)).doc("queue commands"));
