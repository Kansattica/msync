...
parse_result parse(const int argc, const char* argv[], const bool silent)
{
	...
	const auto cli = (newaccount | configMode | syncMode | genMode | queueMode | command("yeehaw").set(ret.selected, mode::yeehaw) | command("version", "--version").set(ret.selected, mode::version) | (command("help").set(ret.selected, mode::help)), universalOptions);
